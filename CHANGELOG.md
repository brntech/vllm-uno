...
- The Gemma 4 profile is certified by the floor-matched instrument that applies to it: greedy output matches plain decoding exactly (4 x 256 tokens), and under sampling Uno is as close to plain as plain is to itself across sessions on this hardware; the kit's permutation gate does not apply to this profile on Ampere because the quantized model's own sampled output is not reproducible across sessions on near-tie tokens (docs/validation.md).
...
- Both profiles are certified by this release; the Gemma 4 sampled certification is the floor-matched reading described above.
