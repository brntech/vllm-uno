...
- Gemma 4 26B A4B support on the Model Runner V2 Uno path: MoE drafting, sliding-window attention, and language-only multimodal admission, shipped as a second serving profile beside Qwen3-8B (`UNO_PROFILE=gemma4`: AWQ 4-bit weights, `TRITON_ATTN`, K=4, `max_num_seqs=4`, `max_model_len=8192`, `gpu_memory_utilization=0.85`, and sixteen base capture cells covering the 16-draft-row bound). Its distributional certification does not close on this build: the floor-matched gate that applies to this profile passes a same-session same-arm floor pair and fails the candidate pair, while the same-arm controls across sessions fail at the same magnitude. See **Verified** and **Scope** below.
- `gates/lossless_floor.py`: the profile evaluation's floor-matched distributional instrument (per prefix and position, plain-versus-plain floor from a same-session same-arm pass pair, Bonferroni cutoff over the tests that ran, permutation budget derived at ten grid steps below that cutoff), shipped beside the permutation comparator with the source identity recorded in `docs/validation.md`.
- A `--chunk` comparison contract in `gates/lossless_spec.py` and `gates/lossless_spec_compare.py`: every sampled pass records the request chunking it ran at, and a comparison — or a floor — declared at a different chunk is refused by name instead of being reported.
...
- The Gemma 4 profile is **not** certified by the floor-matched instrument that applies to it (the kit's permutation gate is not a valid instrument for this profile on this hardware: its first sampled token on the prose prefix is a near-tie whose per-row law moves by more than a nat between the rows of one pass and between launches). Run with the passes interleaved across fresh servers and a same-session same-arm floor pair, the floor pair passes (0 of 36 red, tightest p `0.008528`, max TV `0.773`) and the candidate pair fails (32 of 44 red, min p at the `2.27e-05` grid minimum, max TV `0.930`, 12 advisory TV exceedances) — but the same-arm controls across sessions fail at the same or greater rate: plain against plain 22 of 36, and the Uno arm against itself 37 of 44 against the candidate's 32 of 44. The deviation the gate measures is therefore between launches of the profile rather than between its two arms, so no losslessness claim is supported and no one-sided attribution is made; the profile stays served and documented, uncertified.
...
- The Gemma 4 profile is **not certified** by this release. The instrument that applies to it passes its own same-session same-arm floor pair and fails the candidate pair, while the same-arm controls across sessions are red at the same magnitude; the Qwen3-8B profile is therefore the only profile the v0.3.0 receipts certify. The profile's permutation-gate result from the previous revision of this entry is withdrawn: it compared arms across launches, which this target does not hold stable, and a same-arm pair across launches fails the same way.
